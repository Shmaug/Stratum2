...
namespace tinyvkpt {
	...
	Material Scene::makeMetallicRoughnessMaterial(CommandBuffer& commandBuffer, const ImageValue3& baseColor, const ImageValue4& metallic_roughness, const ImageValue3& transmission, const float eta, const ImageValue3& emission) {
		Material m;
		...
		m.eta() = eta;
		if (baseColor.image || metallic_roughness.image || transmission.image) {
			Descriptors descriptors;

			Image::View d = baseColor.image ? baseColor.image : metallic_roughness.image ? metallic_roughness.image : transmission.image;
			Image::Metadata md;
			md.mExtent = d.extent();
			md.mLevels = Image::maxMipLevels(md.mExtent);
			...
			descriptors[{ "gDiffuse", 0 }] = ImageDescriptor{ baseColor.image ? baseColor.image : d, vk::ImageLayout::eShaderReadOnlyOptimal, vk::AccessFlagBits::eShaderRead, {} };
			...
			Defines defs;
			if (baseColor.image) defs["gUseDiffuse"] = "true";
			if (metallic_roughness.image) defs["gUseSpecular"] = "true";
			if (transmission.image) defs["gUseTransmittance"] = "true";
	...
	Material Scene::makeDiffuseSpecularMaterial(CommandBuffer& commandBuffer, const ImageValue3& diffuse, const ImageValue3& specular, const ImageValue1& roughness, const ImageValue3& transmission, const float eta, const ImageValue3& emission) {
		...
		if (diffuse.image || specular.image || transmission.image || roughness.image) {
			...
			Defines defs;
			if (diffuse.image)      defs["gUseDiffuse"] = "true";
			if (specular.image)     defs["gUseSpecular"] = "true";
			if (transmission.image) defs["gUseTransmittance"] = "true";
			if (roughness.image)    defs["gUseRoughness"] = "true";
	...
	void Scene::update(CommandBuffer& commandBuffer, const float deltaTime) {
		...
		if (mAnimatedTransform) {
			...
			const quatf rotate = (r > 0) ? angle_axis(r * deltaTime, mAnimateRotate / r) : quatf_identity();
		...
		if (!mResources) {
			mResources = make_shared<RenderResources>(commandBuffer.mDevice);
			mResourcePool.emplace(mResources);
		}
		mResources->update(*this, commandBuffer, prevFrame);
	}

	void Scene::RenderResources::update(Scene& scene, CommandBuffer& commandBuffer, const shared_ptr<RenderResources>& prevFrame) {
	...
	void imageValueField(const char* label, ImageValue1& v) {
		...
		if (v.image) {
			...
			if (d)
				...
				ImGui::Image(Gui::getTextureID(v.image), ImVec2(w, w * (float)v.image.extent().height / (float)v.image.extent().width));
			}
		}
	}
	...
	void Camera::drawGui() {
		bool ortho = mProjection.orthographic();
		if (ImGui::Checkbox("Orthographic", reinterpret_cast<bool*>(&ortho)))
			mProjection.vertical_fov = -mProjection.vertical_fov;
		ImGui::DragFloat("Near Plane", &mProjection.near_plane, 0.01f, -1, 1);
		if (mProjection.orthographic()) {
			ImGui::DragFloat("Far Plane", &mProjection.far_plane, 0.01f, -1, 1);
			ImGui::DragFloat2("Projection Scale", mProjection.scale.data(), 0.01f, -1, 1);
		} else {
			float fovy = degrees(2 * atan(1 / mProjection.scale[1]));
			if (ImGui::DragFloat("Vertical FoV", &fovy, 0.01f, 1, 179)) {
				const float aspect = mProjection.scale[0] / mProjection.scale[1];
				mProjection.scale[1] = 1 / tan(radians(fovy / 2));
				mProjection.scale[0] = mProjection.scale[1] * aspect;
			}
		}
		ImGui::DragFloat2("Projection Offset", mProjection.offset.data(), 0.01f, -1, 1);
	...
	void Material::drawGui(Node& node) {
		...
		for (uint i = 0; i < DISNEY_DATA_N; i++)
			if (mValues[i].image) {
				...
				ImGui::Image(Gui::getTextureID(mValues[i].image), ImVec2(w, w * mValues[i].image.extent().height / (float)mValues[i].image.extent().width));
			}
		if (mAlphaMask) {
